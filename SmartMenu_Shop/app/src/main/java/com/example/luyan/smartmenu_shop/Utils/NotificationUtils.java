...
/**
 * Created by luyan on 11/09/2017.
 */
...
public class NotificationUtils {
    ...
    public static Notification configNotify(Object object) {
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(mContext);
        if (object instanceof NOTICEITEM) {
            NOTICEITEM noticeitem = (NOTICEITEM) object;
            mBuilder.setContentTitle(noticeitem.getDeskNum())//设置通知栏标题
                    .setContentText(noticeitem.getNoticeContent())
                    .setContentIntent(getDefalutIntent(Notification.FLAG_AUTO_CANCEL)) //设置通知栏点击意图
//  .setNumber(number) //设置通知集合的数量
                    .setTicker('[' + noticeitem.getDeskNum() + ']' + "发来消息") //通知首次出现在通知栏，带上升动画效果的
                    ...
                    .setSound(Uri.withAppendedPath(MediaStore.Audio.Media.INTERNAL_CONTENT_URI, "5"));//设置通知小ICON
        ...
        if (object instanceof ORDERITEM) {
            ...
            mBuilder.setContentTitle('[' + orderitem.getDeskNum() + ']' + "有订单")//设置通知栏标题
                    .setContentText("查看订单")
                    .setContentIntent(getDefalutIntent(Notification.FLAG_AUTO_CANCEL)) //设置通知栏点击意图
//  .setNumber(number) //设置通知集合的数量
                    .setTicker('[' + orderitem.getDeskNum() + ']' + "有订单") //通知首次出现在通知栏，带上升动画效果的
                    .setWhen(System.currentTimeMillis())//通知产生的时间，会在通知信息里显示，一般是系统获取到的时间
                    .setPriority(Notification.PRIORITY_MAX) //设置该通知优先级
                    .setAutoCancel(true)//设置这个标志当用户单击面板就可以让通知将自动取消
                    .setOngoing(false)//ture，设置他为一个正在进行的通知。他们通常是用来表示一个后台任务,用户积极参与(如播放音乐)或以某种方式正在等待,因此占用设备(如一个文件下载,同步操作,主动网络连接)
